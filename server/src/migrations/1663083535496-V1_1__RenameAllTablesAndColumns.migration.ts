...
export class RenameAllTablesAndColumns1663083535496 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query('ALTER TABLE `attribute` CHANGE `characteristic` `name` VARCHAR(100) NULL');
        ...
        await queryRunner.query('ALTER TABLE `build_components` CHANGE `count_components` `count` INT DEFAULT 1');
        await queryRunner.query('ALTER TABLE `build_components` CHANGE `id_component` `component_id` INT NOT NULL');

        await queryRunner.query('ALTER TABLE `category` CHANGE `id_category` `id` INT NOT NULL');
        ...
        await queryRunner.query('ALTER TABLE `users` CHANGE `photoUrl` `photo` VARCHAR(255) NULL');
    }
