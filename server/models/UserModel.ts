...
export default class UserModel {
    ...
    public async login(user: User): Promise<User> {
        ...
        return new Promise<User>((resolve, reject) => {
            const sql = ` SELECT * FROM users WHERE username=? OR googleId=?`
            ...
            this.pool.execute(sql, data)
                .then((res) => {
                    ...
                    if (!user.googleId && row.length != 0) {
                        const validPassword = bcrypt.compareSync(user.password, row[0].password)
                        if (!validPassword) {
                            reject(UserError.USER_NOT_FOUND)
                        }
                    }
